...
class BedroomConfigurationSupplier(BaseModel):
    """
    BedroomConfigurationSupplier
    """ # noqa: E501
    identifier: UUID = Field(description="Unique identifier")
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Name of layout")
    bedroom_list: Annotated[List[BedroomSupplier], Field(min_length=1)] = Field(description="A room type can have more than one bedroom configuration.", alias="bedroomList")
    __properties: ClassVar[List[str]] = ["identifier", "name", "bedroomList"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in bedroom_list (list)
        _items = []
        if self.bedroom_list:
            for _item_bedroom_list in self.bedroom_list:
                if _item_bedroom_list:
                    _items.append(_item_bedroom_list.to_dict())
            _dict['bedroomList'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "bedroomList": [BedroomSupplier.from_dict(_item) for _item in obj["bedroomList"]] if obj.get("bedroomList") is not None else None
        })
        return _obj
