...
class SyndicationSettingsWithThemeColorsAffiliate(BaseModel):
    """
    ...
    owner_identifier: UUID = Field(description="The user's owner company this entry associates with", alias="ownerIdentifier")
    item_display: Optional[StrictStr] = Field(default=None, description="Whether to treat all links as flat web links or try to embed more advanced data.", alias="itemDisplay")
    customization_identifier: UUID = Field(description="Customization identifier", alias="customizationIdentifier")
    layout_display: Optional[StrictStr] = Field(default='GRID_COLUMNS', description="Which way to itemDisplay the list when WinkLinks first loads", alias="layoutDisplay")
    profile_picture_geometry: Optional[StrictStr] = Field(default='CIRCLE', description="Controls how to itemDisplay profile picture", alias="profilePictureGeometry")
    online_presence_position: Optional[StrictStr] = Field(default='TOP', description="Where to show the online presence icons", alias="onlinePresencePosition")
    qr_code_options: Optional[SyndicationSettingsQrCodeOptionsAffiliate] = Field(default=None, description="Customizable QR code options", alias="qrCodeOptions")
    theme_colors: Optional[CustomizationThemeColorsAffiliate] = Field(default=None, description="Theme colors are connected with the primary account customization.", alias="themeColors")
    __properties: ClassVar[List[str]] = ["id", "createdDate", "lastUpdate", "version", "userIdentifier", "ownerIdentifier", "itemDisplay", "customizationIdentifier", "layoutDisplay", "profilePictureGeometry", "onlinePresencePosition", "qrCodeOptions", "themeColors"]

    @field_validator('item_display')
    def item_display_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['FROM_LAYOUT', 'BANNER', 'BANNER_INTELLIGENT']):
            raise ValueError("must be one of enum values ('FROM_LAYOUT', 'BANNER', 'BANNER_INTELLIGENT')")
        return value

    @field_validator('layout_display')
    def layout_display_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def profile_picture_geometry_validate_enum(cls, value):
        ...
        if value not in set(['CIRCLE', 'RECTANGLE', 'HERO']):
            raise ValueError("must be one of enum values ('CIRCLE', 'RECTANGLE', 'HERO')")
        return value

    @field_validator('online_presence_position')
    def online_presence_position_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of qr_code_options
        if self.qr_code_options:
            _dict['qrCodeOptions'] = self.qr_code_options.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "itemDisplay": obj.get("itemDisplay"),
            "customizationIdentifier": obj.get("customizationIdentifier"),
            "layoutDisplay": obj.get("layoutDisplay") if obj.get("layoutDisplay") is not None else 'GRID_COLUMNS',
            "profilePictureGeometry": obj.get("profilePictureGeometry") if obj.get("profilePictureGeometry") is not None else 'CIRCLE',
            "onlinePresencePosition": obj.get("onlinePresencePosition") if obj.get("onlinePresencePosition") is not None else 'TOP',
            "qrCodeOptions": SyndicationSettingsQrCodeOptionsAffiliate.from_dict(obj["qrCodeOptions"]) if obj.get("qrCodeOptions") is not None else None,
