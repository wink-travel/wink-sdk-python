...
class SearchFiltersNonAuthenticatedEntity(BaseModel):
    """
    SearchFiltersNonAuthenticatedEntity
    """ # noqa: E501
    name_sounds_like: Optional[StrictStr] = Field(default=None, description="A user can query for a property with a name that sounds like", alias="nameSoundsLike")
    radius_in_meters: Optional[StrictInt] = Field(default=0, description="A user can query for blocking that is in a certain proximity to a geo-location. 0 means disabled", alias="radiusInMeters")
    lifestyle: Optional[StrictStr] = Field(default=None, description="A user can filter blocking on a lifestyle")
    star_rating: Optional[StrictInt] = Field(default=None, description="A user can filter on number of hotel stars", alias="starRating")
    direct_only: Optional[StrictBool] = Field(default=None, description="Whether to return blocking that is coming from a supplier with a direct relationship to caller", alias="directOnly")
    __properties: ClassVar[List[str]] = ["nameSoundsLike", "radiusInMeters", "lifestyle", "starRating", "directOnly"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "nameSoundsLike": obj.get("nameSoundsLike"),
