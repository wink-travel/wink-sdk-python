...
class DynamicSellerListRequestAffiliate(BaseModel):
    """
    Paginated search for travel inventory.
    """ # noqa: E501
    list: UpsertDynamicSellerListRequestAffiliate = Field(description="Contains the caller's search criteria.")
    page: Annotated[int, Field(strict=True, ge=0)] = Field(description="Which page to view out of total search results.")
    size: Annotated[int, Field(le=50, strict=True, ge=1)] = Field(description="How many result set to return at the time.")
    display_currency: Optional[Annotated[str, Field(strict=True)]] = Field(default='USD', description="Which currency to display the prices in.", alias="displayCurrency")
    __properties: ClassVar[List[str]] = ["list", "page", "size", "displayCurrency"]

    @field_validator('display_currency')
    def display_currency_validate_regular_expression(cls, value):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "list": UpsertDynamicSellerListRequestAffiliate.from_dict(obj["list"]) if obj.get("list") is not None else None,
            "page": obj.get("page") if obj.get("page") is not None else 0,
            "size": obj.get("size") if obj.get("size") is not None else 24,
