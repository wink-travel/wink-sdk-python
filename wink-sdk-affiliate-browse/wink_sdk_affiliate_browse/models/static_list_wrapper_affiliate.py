...
class StaticListWrapperAffiliate(BaseModel):
    """
    Contains both the curated list and all its items.
    """ # noqa: E501
    list: StaticListAffiliate = Field(description="Curated list object")
    items: Optional[List[StaticListItemAffiliate]] = Field(default=None, description="Travel inventory items contained in list")
    __properties: ClassVar[List[str]] = ["list", "items"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "list": StaticListAffiliate.from_dict(obj["list"]) if obj.get("list") is not None else None,
            "items": [StaticListItemAffiliate.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None
        })
        return _obj
