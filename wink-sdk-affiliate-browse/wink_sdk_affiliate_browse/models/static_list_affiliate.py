...
class StaticListAffiliate(BaseModel):
    """
    StaticListAffiliate
    """ # noqa: E501
    ...
    owner_identifier: UUID = Field(description="List creator", alias="ownerIdentifier")
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Name of curated list")
    type: StrictStr = Field(description="Every affiliate starts out with a `Favorite` list. All other lists will be of type `NORMAL`")
    __properties: ClassVar[List[str]] = ["id", "createdDate", "lastUpdate", "version", "ownerIdentifier", "name", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FAVORITE', 'NORMAL']):
            raise ValueError("must be one of enum values ('FAVORITE', 'NORMAL')")
        return value
