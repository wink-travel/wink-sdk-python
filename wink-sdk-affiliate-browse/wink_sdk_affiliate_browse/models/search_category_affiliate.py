...
class SearchCategoryAffiliate(BaseModel):
    """
    SearchCategoryAffiliate
    """ # noqa: E501
    type: StrictStr = Field(description="Type of search category.")
    key: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Usually the OTA inventory code.")
    value: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Usually the OTA inventory code value.")
    count: Optional[StrictInt] = Field(default=None, description="Quantity of items in this category.")
    identifier: Optional[StrictStr] = Field(default=None, description="UUID.")
    __properties: ClassVar[List[str]] = ["type", "key", "value", "count", "identifier"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['HOTEL', 'INVENTORY']):
            raise ValueError("must be one of enum values ('HOTEL', 'INVENTORY')")
        return value
