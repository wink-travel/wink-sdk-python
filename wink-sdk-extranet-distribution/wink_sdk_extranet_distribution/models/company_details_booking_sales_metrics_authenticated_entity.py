...
class CompanyDetailsBookingSalesMetricsAuthenticatedEntity(BaseModel):
    """
    CompanyDetailsBookingSalesMetricsAuthenticatedEntity
    """ # noqa: E501
    affiliate_account: Optional[BrowseAffiliateAccountAuthenticatedEntity] = Field(default=None, alias="affiliateAccount")
    total: Optional[GroupedBookingSalesMetricsAuthenticatedEntity] = None
    by_property_list: Optional[List[GroupedBookingSalesMetricsAuthenticatedEntity]] = Field(default=None, alias="byPropertyList")
    by_country_list: Optional[List[GroupedBookingSalesMetricsAuthenticatedEntity]] = Field(default=None, alias="byCountryList")
    __properties: ClassVar[List[str]] = ["affiliateAccount", "total", "byPropertyList", "byCountryList"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of affiliate_account
        if self.affiliate_account:
            _dict['affiliateAccount'] = self.affiliate_account.to_dict()
        # override the default output from pydantic by calling `to_dict()` of total
        if self.total:
            _dict['total'] = self.total.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in by_property_list (list)
        _items = []
        if self.by_property_list:
            for _item_by_property_list in self.by_property_list:
                if _item_by_property_list:
                    _items.append(_item_by_property_list.to_dict())
            _dict['byPropertyList'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in by_country_list (list)
        _items = []
        if self.by_country_list:
            for _item_by_country_list in self.by_country_list:
                if _item_by_country_list:
                    _items.append(_item_by_country_list.to_dict())
            _dict['byCountryList'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "affiliateAccount": BrowseAffiliateAccountAuthenticatedEntity.from_dict(obj["affiliateAccount"]) if obj.get("affiliateAccount") is not None else None,
            "total": GroupedBookingSalesMetricsAuthenticatedEntity.from_dict(obj["total"]) if obj.get("total") is not None else None,
            "byPropertyList": [GroupedBookingSalesMetricsAuthenticatedEntity.from_dict(_item) for _item in obj["byPropertyList"]] if obj.get("byPropertyList") is not None else None,
            "byCountryList": [GroupedBookingSalesMetricsAuthenticatedEntity.from_dict(_item) for _item in obj["byCountryList"]] if obj.get("byCountryList") is not None else None
        })
        return _obj
