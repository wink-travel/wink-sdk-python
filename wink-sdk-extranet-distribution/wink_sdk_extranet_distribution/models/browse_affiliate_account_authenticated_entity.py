...
from wink_sdk_extranet_distribution.models.key_value_pair_authenticated_entity import KeyValuePairAuthenticatedEntity
from typing import Optional, Set
from typing_extensions import Self

class BrowseAffiliateAccountAuthenticatedEntity(BaseModel):
    """
    BrowseAffiliateAccountAuthenticatedEntity
    """ # noqa: E501
    identifier: StrictStr
    city: KeyValuePairAuthenticatedEntity
    country: KeyValuePairAuthenticatedEntity
    continent: KeyValuePairAuthenticatedEntity
    name: Annotated[str, Field(min_length=1, strict=True)]
    owner_name: Annotated[str, Field(min_length=1, strict=True)] = Field(alias="ownerName")
    url_name: Annotated[str, Field(min_length=1, strict=True)] = Field(alias="urlName")
    unique_id: Annotated[str, Field(min_length=1, strict=True)] = Field(alias="uniqueId")
    description: Optional[StrictStr] = None
    type: StrictStr
    status: StrictStr
    created_date: datetime = Field(alias="createdDate")
    last_update: datetime = Field(alias="lastUpdate")
    travel_agent: Optional[Any] = Field(alias="travelAgent")
    owner_image_id: StrictStr = Field(description="The company image ID", alias="ownerImageId")
    __properties: ClassVar[List[str]] = ["identifier", "city", "country", "continent", "name", "ownerName", "urlName", "uniqueId", "description", "type", "status", "createdDate", "lastUpdate", "travelAgent", "ownerImageId"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if travel_agent (nullable) is None
        # and model_fields_set contains the field
        if self.travel_agent is None and "travel_agent" in self.model_fields_set:
            _dict['travelAgent'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "city": KeyValuePairAuthenticatedEntity.from_dict(obj["city"]) if obj.get("city") is not None else None,
            "country": KeyValuePairAuthenticatedEntity.from_dict(obj["country"]) if obj.get("country") is not None else None,
            "continent": KeyValuePairAuthenticatedEntity.from_dict(obj["continent"]) if obj.get("continent") is not None else None,
            "name": obj.get("name"),
            ...
            "travelAgent": obj.get("travelAgent"),
