...
class BookingTestRequestSupplierDetails(BaseModel):
    """
    Test booking request body for property ID
    """ # noqa: E501
    query: VerifyRatesRequestSupplierDetails = Field(description="Query object that was used to generate the room property")
    room: RoomConfigurationPriceSupplierDetails = Field(description="The room we want to book")
    announcement: Optional[BookingTestNotificationSupplierDetails] = None
    notification: BookingTestNotificationSupplierDetails = Field(description="Notification options")
    __properties: ClassVar[List[str]] = ["query", "room", "announcement", "notification"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "query": VerifyRatesRequestSupplierDetails.from_dict(obj["query"]) if obj.get("query") is not None else None,
            "room": RoomConfigurationPriceSupplierDetails.from_dict(obj["room"]) if obj.get("room") is not None else None,
            "announcement": BookingTestNotificationSupplierDetails.from_dict(obj["announcement"]) if obj.get("announcement") is not None else None,
            "notification": BookingTestNotificationSupplierDetails.from_dict(obj["notification"]) if obj.get("notification") is not None else None
        })
        return _obj
