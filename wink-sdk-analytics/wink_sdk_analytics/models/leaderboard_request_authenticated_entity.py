...
class LeaderboardRequestAuthenticatedEntity(BaseModel):
    """
    LeaderboardRequestAuthenticatedEntity
    """ # noqa: E501
    page: Annotated[int, Field(strict=True, ge=0)] = Field(description="Which page to access in the record set")
    size: Annotated[int, Field(le=100, strict=True)] = Field(description="Number of result set to retrieve")
    currency_code: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Display leaderboard values in this currency", alias="currencyCode")
    type: StrictStr = Field(description="The type of leaderboard to display")
    grouping_identifier: Optional[StrictStr] = Field(default=None, description="Can be `continent`, `country.geoNameId` or `city.geoNameId`", alias="groupingIdentifier")
    __properties: ClassVar[List[str]] = ["page", "size", "currencyCode", "type", "groupingIdentifier"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GLOBAL', 'CONTINENT', 'COUNTRY', 'CITY']):
            raise ValueError("must be one of enum values ('GLOBAL', 'CONTINENT', 'COUNTRY', 'CITY')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of LeaderboardRequestAuthenticatedEntity from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of LeaderboardRequestAuthenticatedEntity from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'GLOBAL',
            "groupingIdentifier": obj.get("groupingIdentifier")
        })
        return _obj
