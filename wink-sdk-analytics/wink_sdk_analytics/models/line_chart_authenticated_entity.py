...
class LineChartAuthenticatedEntity(BaseModel):
    """
    LineChartAuthenticatedEntity
    """ # noqa: E501
    identifier: Optional[UUID] = Field(default=None, description="Unique line chart identifier")
    title: Optional[ChartTitleAuthenticatedEntity] = Field(default=None, description="The configuration options for the chart title")
    legend: Optional[ChartLegendAuthenticatedEntity] = Field(default=None, description="The configuration options for the chart legend.")
    series: Optional[List[ChartSeriesAuthenticatedEntity]] = Field(default=None, description="The configuration of the chart series. The series type is determined by the value of the type field. If a type value is missing, the chart renders the type that is specified in `seriesDefaults`. Some options accept functions as arguments. These arguments are evaluated for each point which is supplied as a parameter. If no value is returned, the chart uses the `theme` or `seriesDefaults` values.")
    x_axis: Optional[ChartCategoryAxisAuthenticatedEntity] = Field(default=None, description="The configuration options of the category axis.", alias="xAxis")
    xaxis: Optional[ChartCategoryAxisAuthenticatedEntity] = None
    __properties: ClassVar[List[str]] = ["identifier", "title", "legend", "series", "xAxis", "xaxis"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of legend
        if self.legend:
            _dict['legend'] = self.legend.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "title": ChartTitleAuthenticatedEntity.from_dict(obj["title"]) if obj.get("title") is not None else None,
            "legend": ChartLegendAuthenticatedEntity.from_dict(obj["legend"]) if obj.get("legend") is not None else None,
            "series": [ChartSeriesAuthenticatedEntity.from_dict(_item) for _item in obj["series"]] if obj.get("series") is not None else None,
            "xAxis": ChartCategoryAxisAuthenticatedEntity.from_dict(obj["xAxis"]) if obj.get("xAxis") is not None else None,
            "xaxis": ChartCategoryAxisAuthenticatedEntity.from_dict(obj["xaxis"]) if obj.get("xaxis") is not None else None
        })
        return _obj
