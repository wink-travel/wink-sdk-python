...
class SubmitReviewAnswerAuthenticatedEntity(BaseModel):
    """
    SubmitReviewAnswerAuthenticatedEntity
    """ # noqa: E501
    question_identifier: UUID = Field(description="Question identifier", alias="questionIdentifier")
    category: StrictStr = Field(description="Question category")
    sort: StrictInt = Field(description="Sort key")
    value: StrictInt = Field(description="Value")
    __properties: ClassVar[List[str]] = ["questionIdentifier", "category", "sort", "value"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['COMFORT', 'CONDITION', 'SERVICE', 'ENVIRONMENT', 'FACILITIES', 'LOCATION', 'LIFESTYLE', 'VALUE', 'FOOD', 'ROOM']):
            raise ValueError("must be one of enum values ('COMFORT', 'CONDITION', 'SERVICE', 'ENVIRONMENT', 'FACILITIES', 'LOCATION', 'LIFESTYLE', 'VALUE', 'FOOD', 'ROOM')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "questionIdentifier": obj.get("questionIdentifier"),
