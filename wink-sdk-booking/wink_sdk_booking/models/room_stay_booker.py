...
class RoomStayBooker(BaseModel):
    ...
    RoomStayBooker
    """ # noqa: E501
    policy: PropertyPolicyBooker = Field(description="Property policy information.")
    room: GuestRoomLightweightBooker = Field(description="Guest room details.")
    rooms: StrictInt = Field(description="Number of rooms. Always 1 since we switched to creating one booking per room.")
    bedroom_configuration: BedroomConfigurationBooker = Field(description="Desired bedroom layout", alias="bedroomConfiguration")
    ...
    price: StayRateBooker = Field(description="Price details")
    ...
    rate_plan: RoomConfigurationPriceRatePlanBooker = Field(description="Rate plan used for this stay", alias="ratePlan")
    perk_types: Optional[List[PerkLightweightBooker]] = Field(default=None, description="List of perks that came with the master rate", alias="perkTypes")
    extra_charges: ExtraChargesBooker = Field(description="Rate plan-level extra charges", alias="extraCharges")
    active_cancellation_policy: CancellationPolicyLightweightBooker = Field(description="Based on the itinerary, the cancellation policy could be taken directly from the rate plan or it could be a policy exception also listed on the rate plan", alias="activeCancellationPolicy")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "room": GuestRoomLightweightBooker.from_dict(obj["room"]) if obj.get("room") is not None else None,
            ...
            "ratePlan": RoomConfigurationPriceRatePlanBooker.from_dict(obj["ratePlan"]) if obj.get("ratePlan") is not None else None,
            "perkTypes": [PerkLightweightBooker.from_dict(_item) for _item in obj["perkTypes"]] if obj.get("perkTypes") is not None else None,
            "extraCharges": ExtraChargesBooker.from_dict(obj["extraCharges"]) if obj.get("extraCharges") is not None else None,
            "activeCancellationPolicy": CancellationPolicyLightweightBooker.from_dict(obj["activeCancellationPolicy"]) if obj.get("activeCancellationPolicy") is not None else None,
