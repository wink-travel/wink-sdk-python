...
class RealtimeShoppingCartItemAuthenticatedEntity(BaseModel):
    """
    RealtimeShoppingCartItemAuthenticatedEntity
    """ # noqa: E501
    identifier: Annotated[str, Field(min_length=1, strict=True)] = Field(description="identifier for item in cart")
    hotel_identifier: UUID = Field(description="Unique hotel record identifier.", alias="hotelIdentifier")
    start_date: date = Field(description="Date when guest arrives on the premises.", alias="startDate")
    end_date: date = Field(description="Date when guest departs the premises.", alias="endDate")
    room_configuration: RoomConfigurationAuthenticatedEntity = Field(description="Room configuration is how the guest wants the room to support her accompanying guests.", alias="roomConfiguration")
    promotional_codes: Optional[List[StrictStr]] = Field(default=None, alias="promotionalCodes")
    room_rate_identifier: UUID = Field(description="The unique master rate that made the guest room / rate plan available for sale.", alias="roomRateIdentifier")
    ...
    ancillaries: Optional[List[BookingAncillaryAuthenticatedEntity]] = None
    ...
    user: BookingUserRequestAuthenticatedEntity = Field(description="User object contains details of the person that made the booking.")
    hotel: PropertyAggregateLightweightAuthenticatedEntity = Field(description="The hotel where the stay occurs.")
    stay: RoomStayAuthenticatedEntity = Field(description="The priced stay.")
    ...
    language: StrictStr = Field(description="User's language preference")
    currency: StrictStr = Field(description="User's currency preference")
    lifestyle: Optional[StrictStr] = Field(default=None, description="Control which lifestyle context your user was in.")
    ...
    __properties: ClassVar[List[str]] = ["identifier", "hotelIdentifier", "startDate", "endDate", "roomConfiguration", "promotionalCodes", "roomRateIdentifier", "bedroomConfigurationIdentifier", "ancillaries", "specialRequests", "user", "hotel", "stay", "commissionable", "commission", "language", "currency", "lifestyle", "sourceTotal", "userSpecifiedCurrencyTotal", "internalTotal"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ancillaries": [BookingAncillaryAuthenticatedEntity.from_dict(_item) for _item in obj["ancillaries"]] if obj.get("ancillaries") is not None else None,
            ...
            "hotel": PropertyAggregateLightweightAuthenticatedEntity.from_dict(obj["hotel"]) if obj.get("hotel") is not None else None,
            "stay": RoomStayAuthenticatedEntity.from_dict(obj["stay"]) if obj.get("stay") is not None else None,
