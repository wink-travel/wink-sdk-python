...
class PropertyRoomRateWithRateList(BaseModel):
    """
    PropertyRoomRateWithRateList
    """ # noqa: E501
    room: PropertyRoomRate
    rates: Optional[List[PropertyRate]] = None
    start_date: Optional[date] = Field(default=None, alias="startDate")
    end_date: Optional[date] = Field(default=None, alias="endDate")
    __properties: ClassVar[List[str]] = ["room", "rates", "startDate", "endDate"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "room": PropertyRoomRate.from_dict(obj["room"]) if obj.get("room") is not None else None,
            "rates": [PropertyRate.from_dict(_item) for _item in obj["rates"]] if obj.get("rates") is not None else None,
