...
class AddOnLightweightSupplierDetails(BaseModel):
    """
    ...
    location: GeoJsonPointSupplierDetails = Field(description="Geo-location point where blocking takes place. Defaults to location of property.")
    descriptions: Annotated[List[SimpleDescriptionSupplierDetails], Field(min_length=1)] = Field(description="Localized descriptions describing inventory.")
    multimedias: Annotated[List[SimpleMultimediaSupplierDetails], Field(min_length=1)] = Field(description="List of images / videos of blocking.")
    contact: ContactSupplierDetails = Field(description="Associate a contact person for this blocking (if applicable).")
    address: SimpleAddressSupplierDetails = Field(description="Defaults to property address.")
    ...
    socials: Optional[List[SocialSupplierDetails]] = Field(default=None, description="Social network accounts for blocking (if applicable).")
    price_point: StrictStr = Field(description="Level of expensiveness.", alias="pricePoint")
    recognition_list: Optional[List[TravelInventoryRecognitionSupplierDetails]] = Field(default=None, description="Inventory-level recognition.", alias="recognitionList")
    transactional_inventory_list: Optional[List[TransactionalTravelInventorySupplierDetails]] = Field(default=None, description="Purchasable items for this blocking.", alias="transactionalInventoryList")
    ...
    days_of_week: Optional[DowPatternGroupSupplierDetails] = Field(default=None, description="Indicate which days this blocking is open.", alias="daysOfWeek")
    number_of_units: Annotated[int, Field(strict=True, ge=0)] = Field(description="Total number of add-ons available to purchase.", alias="numberOfUnits")
    mandatory: StrictBool = Field(description="Make this add-on mandatory for all guests by enabling this flag.")
    rate_plan_identifier: Optional[StrictStr] = Field(default=None, description="Optional restriction on rate plan.", alias="ratePlanIdentifier")
    __properties: ClassVar[List[str]] = ["identifier", "hotelIdentifier", "featuredInd", "lifestyleType", "location", "descriptions", "multimedias", "contact", "address", "commissionable", "name", "proximityCode", "sort", "minAgeAppropriateCode", "bookable", "active", "disabilityFeatures", "securityFeatures", "socials", "pricePoint", "recognitionList", "transactionalInventoryList", "applicableStart", "applicableEnd", "reservationRequiredInd", "opens", "closes", "daysOfWeek", "numberOfUnits", "mandatory", "ratePlanIdentifier"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "address": SimpleAddressSupplierDetails.from_dict(obj["address"]) if obj.get("address") is not None else None,
            ...
            "daysOfWeek": DowPatternGroupSupplierDetails.from_dict(obj["daysOfWeek"]) if obj.get("daysOfWeek") is not None else None,
            "numberOfUnits": obj.get("numberOfUnits") if obj.get("numberOfUnits") is not None else 0,
            "mandatory": obj.get("mandatory") if obj.get("mandatory") is not None else False,
