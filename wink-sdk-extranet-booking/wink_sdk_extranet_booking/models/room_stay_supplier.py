...
from wink_sdk_extranet_booking.models.guest_room_lightweight_supplier import GuestRoomLightweightSupplier
...
class RoomStaySupplier(BaseModel):
    """
    RoomStaySupplier
    """ # noqa: E501
    policy: PropertyPolicySupplier = Field(description="Property policy information.")
    room: GuestRoomLightweightSupplier = Field(description="Guest room details.")
    rooms: StrictInt = Field(description="Number of rooms. Always 1 since we switched to creating one booking per room.")
    bedroom_configuration: BedroomConfigurationSupplier = Field(description="Desired bedroom layout", alias="bedroomConfiguration")
    ...
    price: StayRateSupplier = Field(description="Price details")
    ...
    rate_plan: RoomConfigurationPriceRatePlanSupplier = Field(description="Rate plan used for this stay", alias="ratePlan")
    perk_types: Optional[List[PerkLightweightSupplier]] = Field(default=None, description="List of perks that came with the master rate", alias="perkTypes")
    extra_charges: ExtraChargesSupplier = Field(description="Rate plan-level extra charges", alias="extraCharges")
    active_cancellation_policy: CancellationPolicyLightweightSupplier = Field(description="Based on the itinerary, the cancellation policy could be taken directly from the rate plan or it could be a policy exception also listed on the rate plan", alias="activeCancellationPolicy")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "room": GuestRoomLightweightSupplier.from_dict(obj["room"]) if obj.get("room") is not None else None,
            "rooms": obj.get("rooms") if obj.get("rooms") is not None else 1,
            "bedroomConfiguration": BedroomConfigurationSupplier.from_dict(obj["bedroomConfiguration"]) if obj.get("bedroomConfiguration") is not None else None,
            ...
            "price": StayRateSupplier.from_dict(obj["price"]) if obj.get("price") is not None else None,
            "roomRateIdentifier": obj.get("roomRateIdentifier"),
            "roomRateInternalName": obj.get("roomRateInternalName"),
            "ratePlan": RoomConfigurationPriceRatePlanSupplier.from_dict(obj["ratePlan"]) if obj.get("ratePlan") is not None else None,
            "perkTypes": [PerkLightweightSupplier.from_dict(_item) for _item in obj["perkTypes"]] if obj.get("perkTypes") is not None else None,
            "extraCharges": ExtraChargesSupplier.from_dict(obj["extraCharges"]) if obj.get("extraCharges") is not None else None,
            "activeCancellationPolicy": CancellationPolicyLightweightSupplier.from_dict(obj["activeCancellationPolicy"]) if obj.get("activeCancellationPolicy") is not None else None,
