...
class InventoryUsageItemSupplier(BaseModel):
    """
    InventoryUsageItemSupplier
    """ # noqa: E501
    usage: StrictStr = Field(description="The type of technology using the inventory")
    count: StrictInt
    ...
    def usage_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['RATE_MODIFIER', 'LINK', 'CARD', 'LIST', 'MAP']):
            raise ValueError("must be one of enum values ('RATE_MODIFIER', 'LINK', 'CARD', 'LIST', 'MAP')")
        return value
