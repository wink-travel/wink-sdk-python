...
class UpsertAddOnRequest(BaseModel):
    """
    UpsertAddOnRequest
    """ # noqa: E501
    ...
    descriptions: Annotated[List[UpsertSimpleDescription], Field(min_length=1)] = Field(description="Localized descriptions describing inventory.")
    multimedias: Annotated[List[SimpleMultimedia], Field(min_length=1)] = Field(description="List of images / videos of blocking.")
    ...
    socials: Optional[List[Social]] = Field(default=None, description="Social network accounts for blocking (if applicable).")
    ...
    rate_plan_identifier: Optional[UUID] = Field(default=None, description="Pass an optional rate plan identifier if you want to add more complex restrictions to this add-on. Example: You want the add-on to only be available when a specific room is available.", alias="ratePlanIdentifier")
    mandatory: StrictBool = Field(description="Make this add-on mandatory for all guests by enabling this flag.")
    transactional_inventory_list: Optional[List[TransactionalTravelInventory]] = Field(default=None, description="Purchasable items for this blocking.", alias="transactionalInventoryList")
    __properties: ClassVar[List[str]] = ["featuredInd", "lifestyleType", "location", "descriptions", "multimedias", "contact", "address", "commissionable", "name", "proximityCode", "sort", "minAgeAppropriateCode", "bookable", "active", "disabilityFeatures", "securityFeatures", "socials", "pricePoint", "recognitionList", "transactionInventoryList", "applicableStart", "applicableEnd", "reservationRequiredInd", "opens", "closes", "daysOfWeek", "numberOfUnits", "ratePlanIdentifier", "mandatory", "transactionalInventoryList"]
