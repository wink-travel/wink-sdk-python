...
class VariableChargeSupplier(BaseModel):
    """
    VariableChargeSupplier
    """ # noqa: E501
    type: StrictStr = Field(description="When the type is FIXED, fixedAmount is required. When the type is PERCENTAGE, percent is required.")
    percent: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="A percentage of the total stay amount for an early check-in or late check-out")
    fixed_amount: Optional[CustomMonetaryAmount] = Field(default=None, description="A localized amount", alias="fixedAmount")
    __properties: ClassVar[List[str]] = ["type", "percent", "fixedAmount"]
