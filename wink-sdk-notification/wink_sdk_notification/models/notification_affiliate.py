...
class NotificationAffiliate(BaseModel):
    """
    ...
    body: StrictStr = Field(description="Body of message")
    ...
    __properties: ClassVar[List[str]] = ["identifier", "ownerIdentifier", "priority", "type", "recipientType", "subject", "body", "read", "markedAsRemoved", "notifyViaEmail"]
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ALERT', 'MESSAGE']):
            raise ValueError("must be one of enum values ('ALERT', 'MESSAGE')")
        return value

    @field_validator('recipient_type')
    def recipient_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['USER', 'COMPANY']):
