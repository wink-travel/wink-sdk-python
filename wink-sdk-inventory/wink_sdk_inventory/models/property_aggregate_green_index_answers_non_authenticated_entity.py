...
class PropertyAggregateGreenIndexAnswersNonAuthenticatedEntity(BaseModel):
    """
    PropertyAggregateGreenIndexAnswersNonAuthenticatedEntity
    """ # noqa: E501
    hotel_identifier: StrictStr = Field(alias="hotelIdentifier")
    list: List[PropertyAggregateGreenIndexAnswerNonAuthenticatedEntity]
    high_score: StrictInt = Field(description="The highest possible score from all questions", alias="highScore")
    total_score: StrictInt = Field(description="The total score from all questions answered", alias="totalScore")
    aggregate_score: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="Total score divided by high score", alias="aggregateScore")
    scores_by_category: List[PropertyAggregateGreenIndexScoreByCategoryNonAuthenticatedEntity] = Field(description="Aggregate scores by Green Index category", alias="scoresByCategory")
    __properties: ClassVar[List[str]] = ["hotelIdentifier", "list", "highScore", "totalScore", "aggregateScore", "scoresByCategory"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.list:
            ...
            _dict['list'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in scores_by_category (list)
        _items = []
        if self.scores_by_category:
            for _item_scores_by_category in self.scores_by_category:
                if _item_scores_by_category:
                    _items.append(_item_scores_by_category.to_dict())
            _dict['scoresByCategory'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "list": [PropertyAggregateGreenIndexAnswerNonAuthenticatedEntity.from_dict(_item) for _item in obj["list"]] if obj.get("list") is not None else None,
            "highScore": obj.get("highScore"),
            "totalScore": obj.get("totalScore"),
            "aggregateScore": obj.get("aggregateScore"),
            "scoresByCategory": [PropertyAggregateGreenIndexScoreByCategoryNonAuthenticatedEntity.from_dict(_item) for _item in obj["scoresByCategory"]] if obj.get("scoresByCategory") is not None else None
        })
        return _obj
