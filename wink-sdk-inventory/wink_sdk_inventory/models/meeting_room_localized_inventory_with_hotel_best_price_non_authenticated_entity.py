...
class MeetingRoomLocalizedInventoryWithHotelBestPriceNonAuthenticatedEntity(BaseModel):
    """
    MeetingRoomLocalizedInventoryWithHotelBestPriceNonAuthenticatedEntity
    """ # noqa: E501
    inventory_with_price: Optional[MeetingRoomLocalizedInventoryNonAuthenticatedEntity] = Field(default=None, description="Meeting room details", alias="inventoryWithPrice")
    hotel_with_best_price: Optional[PropertyWithBestPriceNonAuthenticatedEntity] = Field(default=None, description="Property details along with the best room type price this property has to offer.", alias="hotelWithBestPrice")
    sort: Optional[StrictInt] = Field(default=0, description="Populated when the record is in the context of static lists.")
    __properties: ClassVar[List[str]] = ["inventoryWithPrice", "hotelWithBestPrice", "sort"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of inventory_with_price
        if self.inventory_with_price:
            _dict['inventoryWithPrice'] = self.inventory_with_price.to_dict()
        # override the default output from pydantic by calling `to_dict()` of hotel_with_best_price
        if self.hotel_with_best_price:
            _dict['hotelWithBestPrice'] = self.hotel_with_best_price.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "inventoryWithPrice": MeetingRoomLocalizedInventoryNonAuthenticatedEntity.from_dict(obj["inventoryWithPrice"]) if obj.get("inventoryWithPrice") is not None else None,
            "hotelWithBestPrice": PropertyWithBestPriceNonAuthenticatedEntity.from_dict(obj["hotelWithBestPrice"]) if obj.get("hotelWithBestPrice") is not None else None,
