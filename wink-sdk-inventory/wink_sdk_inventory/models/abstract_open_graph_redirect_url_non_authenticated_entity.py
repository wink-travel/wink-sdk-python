...
class AbstractOpenGraphRedirectUrlNonAuthenticatedEntity(BaseModel):
    """
    AbstractOpenGraphRedirectUrlNonAuthenticatedEntity
    """ # noqa: E501
    type: StrictStr = Field(description="The type of document the uniqueId links to")
    language: StrictStr = Field(description="Which language the seller wanted to use")
    currency: Optional[StrictStr] = Field(default='USD', description="Which currency the seller is using")
    title: StrictStr = Field(description="Link title")
    description: StrictStr = Field(description="Link description")
    ...
    unique_id: StrictStr = Field(description="The URL ID that uniquely represents this link", alias="uniqueId")
    ...
    transact_url: StrictStr = Field(description="The transation url, or where to redirect to when clicking the CTA button.", alias="transactUrl")
    __properties: ClassVar[List[str]] = ["type", "language", "currency", "title", "description", "keywords", "uniqueId", "twitterAccount", "facebookAppId", "image", "video", "transactUrl"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SELLABLE_INVENTORY_SLUG', 'SELLABLE_SUPPLIER_SLUG', 'AFFILIATE_ACCOUNT_SLUG', 'SYNDICATED_ITEM_SLUG', 'URL']):
            raise ValueError("must be one of enum values ('SELLABLE_INVENTORY_SLUG', 'SELLABLE_SUPPLIER_SLUG', 'AFFILIATE_ACCOUNT_SLUG', 'SYNDICATED_ITEM_SLUG', 'URL')")
        return value
