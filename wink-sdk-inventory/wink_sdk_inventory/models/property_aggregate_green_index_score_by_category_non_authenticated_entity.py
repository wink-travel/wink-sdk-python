...
class PropertyAggregateGreenIndexScoreByCategoryNonAuthenticatedEntity(BaseModel):
    """
    PropertyAggregateGreenIndexScoreByCategoryNonAuthenticatedEntity
    """ # noqa: E501
    category: StrictStr = Field(description="Green Index category")
    high_score: StrictInt = Field(description="The highest possible score from all questions within this category", alias="highScore")
    total_score: StrictInt = Field(description="The total score from all questions answered within this category", alias="totalScore")
    aggregate_score: Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]] = Field(description="Total score divided by high score within this category", alias="aggregateScore")
    __properties: ClassVar[List[str]] = ["category", "highScore", "totalScore", "aggregateScore"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GENERAL', 'ENERGY', 'WATER', 'PURCHASING', 'WASTE', 'SITE']):
            raise ValueError("must be one of enum values ('GENERAL', 'ENERGY', 'WATER', 'PURCHASING', 'WASTE', 'SITE')")
        return value
