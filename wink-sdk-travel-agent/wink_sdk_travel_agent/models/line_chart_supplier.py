...
class LineChartSupplier(BaseModel):
    """
    LineChartSupplier
    """ # noqa: E501
    identifier: Optional[UUID] = Field(default=None, description="Unique line chart identifier")
    title: Optional[ChartTitleSupplier] = Field(default=None, description="The configuration options for the chart title")
    legend: Optional[ChartLegendSupplier] = Field(default=None, description="The configuration options for the chart legend.")
    series: Optional[List[ChartSeriesSupplier]] = Field(default=None, description="The configuration of the chart series. The series type is determined by the value of the type field. If a type value is missing, the chart renders the type that is specified in `seriesDefaults`. Some options accept functions as arguments. These arguments are evaluated for each point which is supplied as a parameter. If no value is returned, the chart uses the `theme` or `seriesDefaults` values.")
    x_axis: Optional[ChartCategoryAxisSupplier] = Field(default=None, description="The configuration options of the category axis.", alias="xAxis")
    xaxis: Optional[ChartCategoryAxisSupplier] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "title": ChartTitleSupplier.from_dict(obj["title"]) if obj.get("title") is not None else None,
            "legend": ChartLegendSupplier.from_dict(obj["legend"]) if obj.get("legend") is not None else None,
            "series": [ChartSeriesSupplier.from_dict(_item) for _item in obj["series"]] if obj.get("series") is not None else None,
            "xAxis": ChartCategoryAxisSupplier.from_dict(obj["xAxis"]) if obj.get("xAxis") is not None else None,
            "xaxis": ChartCategoryAxisSupplier.from_dict(obj["xaxis"]) if obj.get("xaxis") is not None else None
        })
        return _obj
