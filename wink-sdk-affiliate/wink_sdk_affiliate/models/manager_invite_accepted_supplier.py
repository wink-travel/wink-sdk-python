...
class ManagerInviteAcceptedSupplier(BaseModel):
    """
    ManagerInviteAcceptedSupplier
    """ # noqa: E501
    id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="AffiliateAccountLightweight ID")
    type: StrictStr = Field(description="Type of account")
    __properties: ClassVar[List[str]] = ["id", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PROPERTY', 'COMPANY', 'ACCOUNT']):
            raise ValueError("must be one of enum values ('PROPERTY', 'COMPANY', 'ACCOUNT')")
        return value
